...
func send(path_store : MUW_Path_Directions_Store, layer : int = 0):
	...
	var path_material = _path_materials_store.get_all()[layer]
		...
		for pos in directions:
			...
			for id in directions[pos]:
				...
				if corners.has(pos):
					...
					corners.erase(pos)
			path_material.update_map(pos, Vector3(path_total, corner_total, 0))

		for pos in corners:
			var corner_total = 0
			for id in corners[pos]:
				corner_total += corners[pos][id]
				path_material.update_map(pos, Vector3(0, corner_total, 0))
